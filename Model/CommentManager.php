...
class CommentManager extends Manager
{
    ...
    {
        $db = $this->dbConnect();
        $query = $db->query('SELECT id, post_id, author, comment, flag_count, DATE_FORMAT(comment_date, \'%d/%m/%Y à %Hh%imin\') AS comment_date_fr FROM comments WHERE flag_count > 0 ORDER BY flag_count DESC');
